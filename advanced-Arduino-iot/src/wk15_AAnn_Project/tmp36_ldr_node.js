...
sp.on('data', function (data) { // call back when data is received
    ...
    firstcommaidx = readData.indexOf(','); // string.indexOf(searchvalue,start)    
    //console.log(data);

    // parsing data into signals
    if (readData.lastIndexOf(',') > firstcommaidx && firstcommaidx > 0) {
        temp = readData.substring(firstcommaidx + 1, readData.lastIndexOf(','));
        lux = readData.substring(readData.indexOf(',', firstcommaidx+1) + 1);
